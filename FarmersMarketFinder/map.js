...
$(function() {
		...
		if (navigator.geolocation) {    
			//this function runs if the request fails and lets you know there's an error
			function error(err) {
				console.warn('ERROR(' + err.code + '): ' + err.message);
			}
			// This is a function to run when the request succeeds.
			...
			// else then alert shows that the geolocation is not supported.
			} else {
		...
		//map options //first initial screen shown 
		// this is the longtitude and latitude and the postioning of the maps.
		var mapOptions = {
			zoom: 5,
	...
	//Adding the infowindow option here. The infowindow is a popup window that is displated above the map at a specific location.
	// The infowindow can be attached to a marker or to a specific latitude/longtitude 
    ...
    $('#chooseZip').submit(function() { // bind function to submit event of form
		...
		//userZip is the variable that will store the input for the users location
		var userZip = $("#textZip").val();
		//console.log("This-> " + userCords.latitude);
		
		var accessURL;
		//if a zipcode exists, the URL we use with the farmers market API uses the zipcode 
		if(userZip){
			accessURL = "http://search.ams.usda.gov/farmersmarkets/v1/data.svc/zipSearch?zip=" + userZip;
		//else, use your Geolocation to grab farmers markets near you	
		} else {
			accessURL = "http://search.ams.usda.gov/farmersmarkets/v1/data.svc/locSearch?lat=" + userCords.latitude + "&lng=" + userCords.longitude;
		}
			 //in order to prevent form from submitting

			//Use the zip code and return all market ids in area.
			//making a GET request using jquery Ajax in order to retrieve the data.
				...
				success: function (data) {
					...
					$.each(marketId, function (k, v){
						$.ajax({
							...
							dataType: 'jsonp',
							success: function (data) {
							// using a for in loop here to iterate over data. 
							for (var key in data) {
